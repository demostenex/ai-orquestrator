...
pub fn ai_memory_project_label() -> String {
    crate::core::config::detect_workspace_dir()
        .ok()
        .and_then(|path| {
            path.file_name()
        ...
        .unwrap_or_else(|| "projeto atual".to_string())
}
...
pub fn sync_handoff_summary(
    ...
    let section = format!(
        ...
        **Dúvidas em aberto:**\n{questions_line}\n",
        ...
        summary = if handoff.summary.is_empty() {
            "(sem resumo)"
        } else {
            &handoff.summary
        },
        next = if handoff.next_action.is_empty() {
